...
├── StrategyDashboard.jsx     # Main application component (refactored)
├── strategy-dashboard-v3.jsx # Original monolithic file (preserved)
...
- `COLORS`: Color scheme for the UI (Bitcoin orange theme)
...
**Why separate:** Makes it easy to update financial data without touching business logic. Configuration changes are isolated.
...
**Why refactored:** Single responsibility - just orchestration and state management.

## Refactoring Details

### Before (strategy-dashboard-v3.jsx)
- **1 file, 1,200+ lines**
- Mixed concerns: API, calculations, components, state
- Hard to test individual functions
- Difficult to locate specific logic
- Prone to naming conflicts

### After (Modular Structure)
- **5 modules, each <500 lines**
...
Ideally, these should have been written first (Test-Driven Development).
...
User Opens App
    ↓
StrategyDashboard.jsx (useEffect)
    ↓
...
See TODO list at bottom of original `strategy-dashboard-v3.jsx`:

1. **STRE Pricing** - Luxembourg Stock Exchange API integration
2. **True VWAP** - Volume-weighted average instead of simple average
3. **Historical Tracking** - Store daily snapshots, show evolution over time
4. **Sensitivity Analysis** - Model MSTR price changes and ITM status
5. **Export/Share** - PDF export, shareable links
6. **STRK Conversion Probability** - Model optionality value
7. **Cost of Capital PV** - Present value of perpetual dividends
8. **Breakeven Calculator** - BTC price for positive common residual
9. **mNAV Comparison** - Market cap / BTC value ratio
10. **Mobile Optimization** - Responsive design improvements
11. **Theme Toggle** - Dark/light mode
12. **WebSocket Integration** - Real-time BTC price updates
13. **Alert System** - Notifications for threshold crossings

## Version History

- **v1**: Initial monolithic implementation
- **v2**: Bug fixes and feature additions
- **v3**: Original working version (preserved as strategy-dashboard-v3.jsx)
- **v3-refactored**: Current modular architecture

## Maintenance Notes

### Updating Financial Data
1. Edit `constants.js` → `STATIC_DATA`
...
1. Add to `STATIC_DATA.preferredStock` in `constants.js`
...
1. Edit function in `calculations.js`
2. Update corresponding test in `calculations.test.js`
...
### Development
- `vitest` - Testing framework (native ES module support, Vite-integrated)
